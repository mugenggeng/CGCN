...
from cgcn_lib.CGCN import CGCN
from cgcn_lib.dataset_fs import VideoDataSet,VideoEpisodicDataSet
from cgcn_lib.loss_function import get_mask, gtad_loss_func # subgraph_loss_func, node_loss_func
...
def train(data_loader, model, optimizer, epoch, bm_mask):
    ...
    for n_iter, (input_data, label_confidence, label_start, label_end) in enumerate(data_loader):
        # forward pass
        # input_data = input_data.to(device=model.devices)
        # bm_mask = bm_mask.to(device=model.devices)
...
def test(data_loader, model, epoch, bm_mask, best_loss):
    ...
    torch.save(state, opt["output"] + "/CGCN_checkpoint.pth.tar")
    if total_am.avg() < best_loss:
        best_loss = total_am.avg()
        torch.save(state, opt["output"] + "/CGCN_best.pth.tar")
...
if __name__ == '__main__':
    ...
    # model = torch.nn.DataParallel(model, device_ids=list([1,2])).cuda()
