...
from cgcn_lib.MGCN import CGCN
from cgcn_lib.dataset_fs import VideoDataSet, VideoEpisodicDataSet
import torch.nn.functional as F
from cgcn_lib.classifier_v2 import Classifier ## changed
from visdom_logger import VisdomLogger
from cgcn_lib.util import AverageMeter, batch_intersectionAndUnionGPU, get_model_dir, main_process
from cgcn_lib.util import find_free_port, setup, cleanup, to_one_hot, intersectionAndUnionGPU
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load(opt["output"] + "/CGCN_best.pth.tar")
    ...
    if valid_mode == "Standard": 
        ...
        # =============== episode = group of tasks ===============
            ...
            # =========== Generate tasks and extract features for each task ===============
                        ...
                        # print("class", classes)#
                        # print(features_s,'features_s1')
                        # print(features_q,'features_q1')
            # =========== Normalize features along channel dimension ===============
                        ## feat_dim : [task=batch, shot=1, c, h, w]
                        if norm_feat:
                            features_s = F.normalize(features_s, dim=1)
                            features_q = F.normalize(features_q, dim=1)
                        # print(features_s, 'features_s2')
                        # print(features_q, 'features_q2')
            # =========== Create a callback is args.visdom_port != -1 ===============
                        callback = VisdomLogger(port=2) if use_callback else None
            # ===========  Initialize the classifier + prototypes + F/B parameter Π ===============                    
                        classifier = Classifier()  
